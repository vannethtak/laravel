...
<div class="{{ config('setting.form_group_class') }}">
    ...
    <div class="{{ config('setting.form_div_input_class') }}">
        <input
            ...
            name="key"
            id="key"
            value="{{ old('key', $data['field_data']->key ?? '') }}"
...
<div class="{{ config('setting.form_group_class') }}">
    ...
    <div class="{{ config('setting.form_div_input_class') }}">
        <input
            ...
            name="value"
            id="value"
            value="{{ old('value', $data['field_data']->value ?? '') }}"
...
<div class="{{ config('setting.form_group_class') }}">
    ...
    <div class="{{ config('setting.form_div_input_class') }}">
        <select
            ...
            required
        >
            <option value="string" {{ old('datatype', $data['field_data']->datatype ?? '') == 'string' ? 'selected' : '' }}>{{ __('string') }}</option>
            <option value="number" {{ old('datatype', $data['field_data']->datatype ?? '') == 'number' ? 'selected' : '' }}>{{ __('number') }}</option>
        </select>
        @error('datatype')
        <div class="invalid-feedback">
            {{ $message }}
